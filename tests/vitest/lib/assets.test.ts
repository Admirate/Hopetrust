...
describe('getAssetUrl', () => {
  ...
  it('returns correct Supabase Storage URL', async () => {
    const { getAssetUrl } = await import('@/lib/assets');
    const url = getAssetUrl('logo1.png');
    expect(url).toBe(
      'https://test-project.supabase.co/storage/v1/object/public/hopetrust%20assets/logo1.png'
    );
  });
  ...
  it('encodes spaces in filenames', async () => {
    ...
    expect(url).toContain('hopetrust%20assets');
    expect(url).toContain('Asset%2010.png');
  });
  ...
  it('handles filenames with special characters', async () => {
    ...
    expect(url).toContain('hopetrust%20assets');
    expect(url).toContain('BACKGROUND%20CIRCLES.png');
  });
});
